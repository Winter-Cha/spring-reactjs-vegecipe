...
public interface BooksRepository extends JpaRepository<Books,Long> {
    ...
    @Query(value = "SELECT b FROM Books b WHERE b.id < ?1 ORDER BY b.id DESC LIMIT 1", nativeQuery = true )
    Books findByIdPreBook(Long id);

    @Query(value = "SELECT b FROM Books b WHERE b.id > ?1 ORDER BY b.id LIMIT 1", nativeQuery = true )
